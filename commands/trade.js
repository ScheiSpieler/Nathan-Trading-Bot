  ...
  const transfer = async (fromId, toId, cards) => {
    for (const { card, quantity } of cards) {
      ...
      const existing = await prisma.userCard.findUnique({
        where: { userId_cardId: { userId: toId, cardId: card.id } },
      });
      if (existing) {
        await prisma.userCard.update({
          where: { userId_cardId: { userId: toId, cardId: card.id } },
        ...
        await prisma.userCard.create({
          data: { userId: toId, cardId: card.id, quantity },
        });
      }
    }
  };
...
module.exports = {
  ...
  execute: async (message, { prisma, tradeSessions }) => {
    ...
    if (tradeSessions.has(senderId))
      return message.reply("⚠️ You're already in a trade session.");

    tradeSessions.set(senderId, {});
    try {
      await message.reply(
        "🔁 Who do you want to trade with? Please `@mention` the user."
      );
      const mentionCollector = message.channel.createMessageCollector({
        filter: (m) => m.author.id === senderId,
        max: 1,
        time: 30000,
      });
      ...
      mentionCollector.on("collect", async (mentionMsg) => {
        ...
        await message.reply(
          "📦 What **cards and quantities** do you want to offer?\nFormat: `Card Name | Quantity`, separate multiple with commas.\nExample: `Cool Nathan | 2, Pixel Nathan | 1`"
        );

        const offerCollector = message.channel.createMessageCollector({
          filter: (m) => m.author.id === senderId,
          max: 1,
          time: 60000,
        });
        ...
        offerCollector.on("collect", async (offerMsg) => {
          const offerList = offerMsg.content
            .split(",")
            .map((s) => s.trim())
            .map((s) => {
          ...
          if (
            offerList.some(
              (e) => !e.name || isNaN(e.quantity) || e.quantity < 1
            )
          ) {
            tradeSessions.delete(senderId);
            return message.reply(
              "❌ Invalid format in one or more entries. Use `Card Name | Quantity`. Trade canceled."
            );
          }
          ...
          for (const offer of offerList) {
            ...
            if (!card)
              return message.reply(
                `❌ Card not found: ${offer.name}. Trade canceled.`
              );

            const userCard = await prisma.userCard.findUnique({
              where: { userId_cardId: { userId: sender.id, cardId: card.id } },
            });
            if (!userCard || userCard.quantity < offer.quantity)
              return message.reply(
                `❌ You don't have enough of: ${offer.name}. Trade canceled.`
              );
          ...
          await message.reply(
            "🎁 What **cards and quantities** do you want **from ${targetUser.username}**?\nFormat: `Card Name | Quantity`, separate multiple with commas.\nExample: `Cool Nathan | 2, Pixel Nathan | 1`"
          );

          const requestCollector = message.channel.createMessageCollector({
            filter: (m) => m.author.id === senderId,
            max: 1,
            time: 60000,
          });
          ...
          requestCollector.on("collect", async (reqMsg) => {
            const requestList = reqMsg.content
              .split(",")
              .map((s) => s.trim())
              .map((s) => {
                const [name, qtyStr] = s.split("|").map((t) => t.trim());
            ...
            for (const req of requestList) {
              ...
              if (!card)
                return message.reply(
                  `❌ Card not found: ${req.name}. Trade canceled.`
                );

              const userCard = await prisma.userCard.findUnique({
                where: {
                  userId_cardId: { userId: receiver.id, cardId: card.id },
                },
              });
              if (!userCard || userCard.quantity < req.quantity)
                return message.reply(
                  `❌ ${targetUser.username} doesn't have enough of: ${req.name}. Trade canceled.`
                );
            ...
            buttonCollector.on("collect", async (interaction) => {
              if (interaction.user.id !== targetUser.id)
                return interaction.reply({
                  content: "This trade isn't for you.",
                  ephemeral: true,
                });
              ...
              if (!stillValid) {
                ...
                return interaction.update({
                  content:
                    "❌ One or both users no longer have the required cards.",
                  embeds: [],
                  components: [],
                });
              }
              ...
              await checkFirstFullCompletion(
                prisma,
                message.author,
                message.client
              );
              ...
              await prisma.trade.create({
                data: {
                  ...
                  offered: session.offer.map((e) => ({
                    cardId: e.card.id,
                    quantity: e.quantity,
                  })),
                  requested: session.request.map((e) => ({
                    cardId: e.card.id,
                    quantity: e.quantity,
                  })),
                },
