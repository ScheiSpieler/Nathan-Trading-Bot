...
module.exports = {
    ...
    execute: async (message, { prisma, ADMIN_IDS }) => {
        ...
        const [rarity, name, imageUrlInput] = rawArgs.split(",").map(arg => arg.trim());
        ...
        if (!CardRarity[upper]) {
            return message.reply(`❌ **Invalid rarity.** Valid options: ${validRarities}`);
        }

        try {
            const card = await prisma.card.create({ data: { name, imageUrl, rarity: upper } });
